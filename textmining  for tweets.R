...
cred <- OAuthFactory$new(consumerKey='gY36Ia5KHJ2ipGfyZBG8XEa7a',
                         consumerSecret='hilYhaJShz9If5hOUnP3Mx3zOEzKiqFkvpW7omjNpdZoKFtJwo',
...
setup_twitter_oauth("gY36Ia5KHJ2ipGfyZBG8XEa7a", 
                    "hilYhaJShz9If5hOUnP3Mx3zOEzKiqFkvpW7omjNpdZoKFtJwo",
                    "1271453422177288197-CoM7PI5TAUr2Kyhyj8ir831oRHmyqP", # Access token
                    "njkxFTwlE6uzFTcUE8FE5vXch7LxVHsAlGQFcXMwI4RlT")  # Access token secret key
...
write.csv(TweetsDF,"BillgatesTweets.csv",row.names = F)
getwd()
...
for (i2 in 1:max(a3$cluster)){	
  makewordc(a3[[i2]])	
  sub=paste("wordcloud-Clustering-",as.character(i2),"-",as.character(format(round(ncol(a3[[i2]])*100/nrow(dtm0),2),nsmall=3)),"%",collapse = " ")
  title(sub = sub)
