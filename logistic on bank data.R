...
library(dplyr)
banks<-read.csv(file.choose(),sep=';',header = T)
View(banks)
summary(banks)
...
p1=gg+geom_histogram(aes(x=age),color="black",fill="white",binwidth = 5)+
  ggtitle("Age Distribution(red mean line ")+ylab("Count")+xlab("Age")+
  geom_vline(aes(xintercept = mean(age),color="red"))+scale_x_continuous(breaks = seq(0,100,5))+
...
#Having higher education is seen to contribute to higher subscription of term deposit.
#Most clients who subscribe are from 'secondary' and 'tertiary' education levels. 
#Tertiary educated clients have higher rate of subscription (15%) from total clients called.
...
#Duration on call similar for first 10 contacts during campaign. 
#Successful subscription (y=1) occur within first 10 contacts. Much less after that.
...
#Scatterplot Matrix
#Due to large number of attributes (17 total), 8 was chosen for correlation. 
#No clear correlation pattern can be observed as most attributes are categorical.
...
plot_pred_type_distribution <- function(df, threshold) {
 ...
 #model 
#build model by fitting logistic regression on train data set
