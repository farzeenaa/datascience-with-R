# -*- coding: utf-8 -*-
"""
Created on Fri Dec  4 13:57:54 2020
...
def week():
	if request.method == 'POST':
	  	...
	  	band=pd.DataFrame()
	  	band['REVIEWS']=total_review
	  	band['RATINGS']=total_rating
	  	band['DATE']=total_date
	  	band['RATINGS']=band['RATINGS'].replace({'1':'VERY NEGATIVE','2':'NEGATIVE','3':'NEUTRAL','4':'POSITIVE','5':'VERY POSITIVE'})
	  	lemmit= WordNetLemmatizer()
	  	tokens=[]
	  	for i in range(len(band)):
	  		review = re.sub('[^a-zA-Z]', ' ', band['REVIEWS'][i])
	  	...
	  	for x in range(len(tokens)):
	  		micro=band.replace(to_replace=band['REVIEWS'][x],value=tokens[x])
	  	Data=pd.DataFrame()
	  	...
	  	for i in range(1,8):
	  		...
	  		if previous_date[0]==str(0):
	  			...
	  			pass
	  		x=band.loc[band['DATE']==previous_date,'REVIEWS']
	  		z=[]
	  		if len(x)>0:
	  			z=stack_bal.predict(trans.transform(band.loc[micro['DATE']==previous_date,'REVIEWS']))
	  			for y in z:
	  				week_data.append(y)
	  			for a in x:
